...
import type { PublicStatsResult } from "../../../ai/tools/weather/tool";
  ...
  React.useEffect(() => {
    ...
    const fetchLive = async () => {
      try {
        ...
        const res = await fetch(`https://earthquake.usgs.gov/fdsnws/event/1/query?${params.toString()}`, { signal: controller.signal });
      ...
      <CardContent>
        <ChartContainer config={config} className="aspect-auto h-[300px]">
          <AreaChart data={chartData} margin={{ left: 8, right: 8, top: 8, bottom: 8 }}>
            ...
            <YAxis tickFormatter={(v) => Intl.NumberFormat(undefined, { notation: "compact" }).format(Number(v))} tick={{ fontSize: 12 }} tickMargin={8} />
            <ChartTooltip content={(props: TooltipContentProps<ValueType, NameType>) => <ChartTooltipContent {...props} />} />
            <ChartLegend verticalAlign="top" content={(props: DefaultLegendContentProps) => <ChartLegendContent verticalAlign="top" payload={props.payload} />} />
            <Area
              type="monotone"
              dataKey="count"
